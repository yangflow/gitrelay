...
if [[ ! -x "$CLI_BIN" ]]; then
    echo "error: gitrelayctl missing at $CLI_BIN (build the gitrelay scheme so the Copy CLI phase runs)" >&2
    exit 1
fi
echo "==> Verified bundled CLI: $CLI_BIN"
...
echo ""
echo "  2. Update the Homebrew tap:"
echo "     ./scripts/update-tap.sh $VERSION"
