...
The mark is a purple squircle carrying a white Y-shaped git branch. Its geometry
mirrors `GitRelayCore/Design/GitRelayMark.swift`, which the menu-bar status item
also draws from — as a monochrome template, without the purple plate. Change the
numbers in both places; `GitRelayMarkTests` fails if they drift apart.
